...
[ApiVersion("1.0")]
[Route("api/v{version:apiVersion}/prepayments")]
[Authorize(AuthenticationSchemes = JwtBearerDefaults.AuthenticationScheme, Roles = "Administrator, Auxiliar")]
[ApiController]
public class PrePaymentsController : ControllerBase
{
    private readonly IPaymentServiceX _paymentService;
    private readonly IStringLocalizer _localizer;

    public PrePaymentsController(IPaymentServiceX paymentService, IStringLocalizer localizer)
    {
    ...
    public async Task<IActionResult> GetServicesAsync(Guid contractClientId, [FromQuery] Guid? prePaymentId = null)
    {
        ...
        {
            ClaimsDTOs userClaimsInfo = User.GetSecurityContextOrThrow(_localizer, HttpContext);
            var response = await _paymentService.GetPrePaymentServicesAsync(contractClientId, prePaymentId, userClaimsInfo.UserName);
            return ResponseHelper.Format(response);
        }
        catch (ApplicationException ex)
        {
