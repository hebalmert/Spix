...
public class CargueService : ICargueService
{
    ...
    public async Task<ActionResponse<IEnumerable<Cargue>>> GetAsync(PaginationDTO pagination, string username)
    {
        try
        {
    ...
    public async Task<ActionResponse<Cargue>> AddAsync(Cargue modelo, string username)
    {
        await _transactionManager.BeginTransactionAsync();
        try
        {
