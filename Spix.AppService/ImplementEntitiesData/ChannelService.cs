...
public class ChannelService : IChannelService
{
    ...
    public async Task<ActionResponse<IEnumerable<IntItemModel>>> ComboAsync()
    {
        try
        {
            ...
            List<IntItemModel> ListModel = new(await _comboCache.GetOrCreateAsync("combo_channels", async () =>
                await _context.Channels.AsNoTracking().Where(x => x.Active).Select(c => new IntItemModel()
                {
                    Name = c.ChannelName,
                    Value = c.ChannelId
                }).ToListAsync()));
