...
public class ChainTypesService : IChainTypesService
{
    ...
    public async Task<ActionResponse<IEnumerable<IntItemModel>>> ComboAsync()
    {
        try
        {
            ...
            List<IntItemModel> ListModel = new(await _comboCache.GetOrCreateAsync("combo_chaintypes", async () =>
                await _context.ChainTypes.AsNoTracking().Where(x => x.Active).Select(c => new IntItemModel()
                {
                    Name = c.ChainName,
                    Value = c.ChainTypeId
                }).ToListAsync()));
