...
//
// POR QUE HAY UNA COPIA: el dibujado es identico (pdf.js, los recuadros, el arrastre), lo
// unico que cambia es COMO le avisa a C#. En Blazor el JS llama a
// dotNetRef.invokeMethodAsync("PlaceField", ...); en el WPF no hay DotNetObjectReference:
// el navegador embebido habla con la aplicacion por window.chrome.webview.postMessage y del
// otro lado escucha CoreWebView2.WebMessageReceived. Se cambiaron SOLO esas tres llamadas;
// los datos y sus nombres son los mismos.
//
// Vive como cadena y no como archivo suelto para no tener que tocar el .csproj: se publica
// en tiempo de ejecucion con WebViewEnvironment.PublicarDocumento("pdfFieldEditor.js", Codigo).
public static class PdfFieldEditorScript
{
    public const string Codigo = """
        // Editor visual de campos para Plantillas PDF.
        // Dibuja las paginas con pdf.js y encima los recuadros de los campos. La aplicacion manda la lista
        // de campos (setFields) y el editor avisa cada accion del usuario (PlaceField, MoveField, SelectField).
        ...
        (function () {
            ...
            // El unico cambio respecto de la web: asi le avisa el navegador embebido a la aplicacion.
            ...
            async function load(hostId, base64, readOnly) {
                dispose();
                ...
                const token = {};
                state = { token, host, readOnly, pdf: null, pages: [], fields: [], selectedKey: null, placingType: 0, drag: null };
                host.innerHTML = "";
            ...
            function placeField(e, pageNumber) {
                ...
                avisar({ accion: "PlaceField", pageNumber: pageNumber, positionX: position.x, positionY: position.y });
            }
            ...
            function dragEnd() {
                ...
                if (!drag.moved) {
                    avisar({ accion: "SelectField", key: drag.field.key });
                    return;
                }
                ...
                avisar({
                    ...
                    width: isSignature ? box.width : null,
                    height: isSignature ? box.height : null
                });
            }
