...
public partial class IndexTechnitian
{
    ...
    private const string baseUrl = "api/v1/technitians";
    ...
    private async Task Cargar(int page = 1)
    {
        ...
        var responseHttp = await _repository.GetAsync<List<Technician>>(url);
        ...
        Technicians = responseHttp.Response;
    ...
    private async Task ShowModalAsync(Guid? id = null, bool isEdit = false)
    {
        ...
        if (isEdit)
        {
            component = typeof(EditTechnitian);
            parameters = new Dictionary<string, object>
                {
                    { "Id", id! },
                    { "Title", $"{Localizer[nameof(Resource.Edit_Technitian)]}"  }
                };
        }
        else
        {
            component = typeof(CreateTechnitian);
            parameters = new Dictionary<string, object>
                {
                    { "Title", $"{Localizer[nameof(Resource.Create_Technitian)]}"  }
                };
        }
    ...
    private async Task ShowAuditAsync(Technician item)
    {
