...
public partial class IndexSoftplan
{
    ...
    private async Task ShowModalAsync(int id = 0, bool isEdit = false)
    {
        ...
        if (isEdit)
        {
            component = typeof(EditSoftPlan);
            parameters = new Dictionary<string, object>
        {
            { "Id", id! },
            { "Title", $"{Localizer[nameof(Resource.Edit_SoftPlan)]}" }
        };
        }
        else
        {
            component = typeof(CreateSoftPlan);
            parameters = new Dictionary<string, object>
        {
            { "Title", $"{Localizer[nameof(Resource.Create_SofPlan)]}" }
        };
        }
