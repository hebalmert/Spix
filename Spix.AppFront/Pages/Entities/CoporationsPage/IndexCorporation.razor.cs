...
public partial class IndexCorporation
{
    ...
    private async Task ShowModalAsync(int id = 0, bool isEdit = false)
    {
        ...
        if (isEdit)
        {
            component = typeof(EditCorporation);
            parameters = new Dictionary<string, object>
        {
            { "Id", id! },
            { "Title", $"{Localizer[nameof(Resource.Edit_Corporation)]}" }
        };
        }
        else
        {
            component = typeof(CreateCorporation);
            parameters = new Dictionary<string, object>
        {
            { "Title", $"{Localizer[nameof(Resource.Create_Corportation)]}" }
        };
        }
