...
public partial class EditPrePayment
{
    ...
    {
        var responseHttp = await _repository.GetAsync<PrePayment>($"{BaseUrl}/{Id}");
        if (await _responseHandler.HandleErrorAsync(responseHttp))
        {
    ...
    private static BillingContractDto? BuildContractDto(PrePayment? model)
    {
