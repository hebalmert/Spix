...
public partial class EditContractExonerated
{
    ...
    private ContractExonerated? Model;
    ...
    {
        var responseHttp = await _repository.GetAsync<ContractExonerated>($"{BaseUrl}/{Id}");
        if (await _responseHandler.HandleErrorAsync(responseHttp))
        {
    ...
    private static BillingContractDto? BuildContractDto(ContractExonerated? model)
    {
