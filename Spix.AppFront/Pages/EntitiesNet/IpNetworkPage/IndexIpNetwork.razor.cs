...
public partial class IndexIpNetwork
{
    ...
    private async Task ShowModalAsync(Guid? id = null, bool isEdit = false)
    {
        ...
        if (isEdit)
        {
            component = typeof(EditIpNetwork);
            parameters = new Dictionary<string, object>
                {
                    { "Id", id! },
                    { "Title", $"{Localizer[nameof(Resource.Edit_Ip)]}"  }
                };
        }
        else
        {
            component = typeof(CreateIpNetwork);
            parameters = new Dictionary<string, object>
                {
    ...
    private async Task ShowPoolModalAsync()
    {
        ...
        await _modalService.ShowAsync(typeof(CreateIpNetworkPool), parameters, async result =>
        {
            if (result.Succeeded)
            {
    ...
    private async Task ShowDeletePoolModalAsync()
    {
        ...
        await _modalService.ShowAsync(typeof(DeleteIpNetworkPool), parameters, async result =>
        {
            if (result.Succeeded)
            {
                await Cargar(CurrentPage);
