...
public class ManagerService : IManagerService
{
    ...
    public async Task<ActionResponse<Manager>> GetAsync(int id)
    {
        ...
        {
            return await _httpErrorHandler.HandleErrorAsync<Manager>(ex);
        }
    }
    ...
    public async Task<ActionResponse<Manager>> AddAsync(Manager Newmodelo, string frontUrl)
    {
        ...
        try
        {
            if (await _userHelper.GetUserByUserNameAsync(Newmodelo.UserName) is not null)  //---> Validacion de UserName Unico
            {
                return new ActionResponse<Manager>
                {
                    WasSuccess = true,
            ...
            if (await _userHelper.GetUserByEmailAsync(Newmodelo.Email) is not null) //---> Validacion de Email Unico
            {
                return new ActionResponse<Manager>
                {
                    WasSuccess = true,
