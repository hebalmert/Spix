...
public class FrecuencyTypeService : IFrecuencyTypeService
{
    ...
    public async Task<ActionResponse<IEnumerable<IntItemModel>>> ComboAsync()
    {
        try
        {
            List<IntItemModel> ListModel = await _context.FrecuencyTypes.Where(x => x.Active)
                .Select(x => new IntItemModel { Name = x.TypeName, Value = x.FrecuencyTypeId }).ToListAsync();
